...
| `fibo-ext-temporal-2024Q1.owl` | 14 KB | Custom FIBO extensions (OWL/RDF) |
...
### 2. FIBO Extensions (`fibo-ext-temporal-2024Q1.owl`)
...
4. **Regulatory Compliance**
   ...
   - GDPR Article 22, SEC Regulation S-K
...
6. **Market Regime Context**
   - `MarketRegime` class
   - 3 regimes (HMM states, Section 3.4.5): BullMarket, BearMarket, SidewaysMarket
...
**Thresholds (Table 1):** causal edges are kept above τ_causal = 0.70; after the
contradiction penalty, edges below τ_min = 0.35 are removed.
...
|-----------|--------|-------|
| **FIBO Integration** | Classes / properties (3 modules) | 188 / 363 |
| | Entity-mention coverage | ~78% |
| **Knowledge Graph** | Entity nodes / edges | ~2.4M / 8.7M |
| | Average query time | 127 ± 18 ms |
| **Causal Extraction** | Causal relationship F1 (Table 9) | 69.7 ± 4.3% |
| **Temporal** | TCC (Table 4) | 0.54 ± 0.07 |
| | HMM regime accuracy (held-out, 2020–2024) | 81.3% — HMM not included in this release |
...
- **FIBO coverage:** ~78% of financial entity mentions
...
Load in Protégé:
1. Open Protégé
2. File → Open → Select `fibo-ext-temporal-2024Q1.owl`
